---
title: "Untargeted characterization of Amazonian barks extracts plants by Liquid and Gas Chromatography - Mass Spectrometry | Liquid Chromatography - Mass Spectrometry (LC-MS) data analysis"
...
data <- read_from_excel(file = "Data/LCMS_to_R.xlsx", sheet = 4, 
...
ggplot(scores,
  ...
  geom_point(size = 3) +
  guides(x=guide_axis(title = "PC1 (28.73 %)"),
         y=guide_axis(title = "PC2 (18.10 %)")) +
  theme_classic()
# Save plot
...
loadings <- neg_pca$rotation %>%           # Extract loadings
  data.frame(Feature_ID = rownames(.))     # New column with feature name
...
#write_to_excel(imputed, "Result/LCMS/to_LCMS_Metabolites.xlsx")
# Load a metabolite name table
#metab_name <- readxl::read_excel("Data/LCMS_Metabolites.xlsx", 5)
# Creating a new small table of the annotated compounds
#neg_compouds <- left_join(metab_name, loadings)
# Metabolite identification label
`Identification level` <- feat_data$Identification_level
# Plotting results
ggplot(loadings, aes(PC1, PC2)) + 
  geom_point(alpha = 0.2) +
  theme_classic() + 
  geom_point(data = loadings, aes(shape = `Identification level`, color = `Identification level`), size = 2) +
  ggrepel::geom_label_repel(data = loadings,
                            aes(label = feat_data$Metabolite_name),
                            box.padding = 0.8,
                            label.padding = 0.3,
                            label.r = 0.3,
                            cex = 3) +
...
# Save plot
ggsave('Result/LCMS/LCMS_Loadings.png', width = 5, height = 4, device='png', dpi="print")
...
raw_hm <- corrected_no_blank[!is.na(corrected_no_blank@featureData@data$Metabolite_name),]
# Drop QC
raw_hm <- drop_qcs(raw_hm)
# Data imputation
set.seed(201)
raw_hm <- impute_simple(raw_hm, value = "half_min", na_limit = 0.8)
raw_hm <- impute_rf(raw_hm, all_features = TRUE)
# Extracting feature height table
...
cols_metclass <- c("Organic oxygen compounds" = "#800000FF",
                   "Organic acids and derivatives" = "#8A9045FF",
                   "Benzenoids" = "#FFA319FF",
                   "Phenylpropanoids and polyketides" = "#FF5A5A",
                   "Lipids and lipid-like molecules" = "#155F83FF")
